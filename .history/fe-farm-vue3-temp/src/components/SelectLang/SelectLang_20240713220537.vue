...
export default defineComponent({
  name: 'SelectLang',
  setup() {
    const selectedLocale = ref(''); // 默认选中的语言的 code，将在此处初始化
    const activeIndex = ref(0); // 用于追踪激活的<li>的索引
    const localeMenuButtonRef = ref<HTMLElement | null>(null); // 创建一个ref来引用DOM元素

    // 初始化 selectedLocale 为 "简体中文" 的 code
    const defaultLanguage = languages.find(lang => lang.cnName === '简体中文');
    if (defaultLanguage) {
      selectedLocale.value = defaultLanguage.code;
    }

    const changeLocale = (language: typeof languages[number], index: number) => {
      selectedLocale.value = language.code;
      activeIndex.value = index;
      updateLocaleButton(); // 更新按钮属性
    };
    ...
    const updateLocaleButton = () => {
      ...
      const button = localeMenuButtonRef.value;
      if (button) {
        button.dataset.locale = selectedLocale.value;
        button.title = selectedLocale.value;
      }
    };
    ...
    onMounted(() => {
      // 初始化时设置默认的locale和activeIndex
      activeIndex.value = languages.findIndex(lang => lang.code === selectedLocale.value);
    });
...
<template>
  <div id="locale" class="bg-color dropdown" data-component="Locale">
    <a ref="localeMenuButton" id="localeMenuButton" role="menuitem" tabindex="0" aria-haspopup="true" aria-controls="localeMenuList" aria-label="Localization" aria-expanded="false" class="locale-icon focusable text-color-hover" data-component="LocaleIcon" :data-locale="selectedLocale" :title="selectedLocale">
      <i class="eg-header-icon-locale"></i>
    </a>
    <ul id="localeMenuList" class="locale-list bg-color ul-left" role="menu" aria-labelledby="localeMenuButton">
      <li role="none" class="" v-for="(language, index) in languages" :key="language.code" :data-lang="language.code" :class="{ active: index === activeIndex }">
